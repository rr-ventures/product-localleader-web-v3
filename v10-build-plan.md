# Local Leader — V10 Build Plan

> **For:** Gemini 3.1 Pro build agent
> **Goal:** Build a complete, production-quality landing page at `/v10` using the reference screenshots as visual inspiration and this document as the content/structural guide.
...
| Version | v10 |
| Route | `/v10` |
| Layout | `src/layouts/LayoutV10.astro` (already exists with Inter + Playfair Display loaded) |
| Page | `src/pages/v10/index.astro` (replace current scaffold content) |
| Components | `src/components/v10/` (create component files here) |
| Thank-you page | `src/pages/v10/thank-you.astro` (create this) |
| Font pairing | Inter (sans) + Playfair Display (serif) -- already configured in the layout |

The layout scaffold at `src/layouts/LayoutV10.astro` already has fonts, meta tags, and the HTML shell. Do not modify it unless you need to add something to the `<head>`. Build everything else as components imported into the page file.

---

## 2. REFERENCE IMAGES

**Folder:** `_staging/references/v3/` (using v3 references as inspiration for this v10 build)
...
- Links to: `/v10/thank-you`
- Lower friction entry point for prospects not ready to book a call

**Thank-you page (`src/pages/v10/thank-you.astro`):**
- Create this page matching the v10 design aesthetic
...
- Include a "Back to home" link to `/v10`
...
Copy your chosen logo to `public/images/` with a version-prefixed name (e.g., `v10-logo.jpg`).
...
- **Architecture:** Self-contained version. Own layout, own components in `src/components/v10/`. Do not import from `src/components/sections/` or `src/components/v2/` — those belong to other versions.
- **Layout:** The layout scaffold already exists at `src/layouts/LayoutV10.astro` with fonts loaded. Replace the page content, do not modify the layout unless you need to add something to the `<head>`.
...
1. Update `VERSIONS.md` in the project root:
   - Change v10 status from "Scaffold" to "Complete"
...
2. Update the hub page `src/pages/index.astro`:
   - Change the v10 card badge from "Scaffolded" to "Live"
