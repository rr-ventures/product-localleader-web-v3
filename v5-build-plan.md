# Local Leader — V5 Build Plan

> **For:** Gemini 3.1 Pro build agent
> **Goal:** Build a complete, production-quality landing page at `/v5` using the reference screenshots as visual inspiration and this document as the content/structural guide.
...
| Version | v5 |
| Route | `/v5` |
| Layout | `src/layouts/LayoutV5.astro` (already exists with Outfit + Source Serif 4 loaded) |
| Page | `src/pages/v5/index.astro` (replace current scaffold content) |
| Components | `src/components/v5/` (create component files here) |
| Thank-you page | `src/pages/v5/thank-you.astro` (create this) |
| Font pairing | Outfit (sans) + Source Serif 4 (serif) — already configured in the layout |

The layout scaffold at `src/layouts/LayoutV5.astro` already has fonts, meta tags, a warm off-white background (`#fbfaf7`), and the HTML shell. Do not modify it unless you need to add something to the `<head>`. Build everything else as components imported into the page file.

---

## 2. REFERENCE IMAGES

**Folder:** `_staging/references/v5/`

Files: `v5a.png`, `v5b.png`, `v5c.png`, `v5d.png`, `v5e.png`, `v5f.png`
...
- Links to: `/v5/thank-you`
- Lower friction entry point for prospects not ready to book a call

**Thank-you page (`src/pages/v5/thank-you.astro`):**
- Create this page matching the v5 design aesthetic
...
- Include a "Back to home" link to `/v5`
...
- **Architecture:** Self-contained version. Own layout, own components in `src/components/v5/`. Do not import from `src/components/sections/`, `src/components/v2/`, `src/components/v3/`, or `src/components/v4/` — those belong to other versions.
- **Layout:** The layout scaffold already exists at `src/layouts/LayoutV5.astro` with fonts loaded. Replace the page content, do not modify the layout unless you need to add something to the `<head>`.
...
1. Update `VERSIONS.md` in the project root:
   - Change v5 status from "Scaffold" to "Complete"
...
2. Update the hub page `src/pages/index.astro`:
   - Change the v5 card badge from "Scaffolded" to "Live"
