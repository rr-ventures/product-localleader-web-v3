# Local Leader — V3 Build Plan

> **For:** Gemini 3.1 Pro build agent
> **Goal:** Build a complete, production-quality landing page at `/v3` using the reference screenshots as visual inspiration and this document as the content/structural guide.
...
| Version | v3 |
| Route | `/v3` |
| Layout | `src/layouts/LayoutV3.astro` (already exists with DM Sans + Lora loaded) |
| Page | `src/pages/v3/index.astro` (replace current scaffold content) |
| Components | `src/components/v3/` (create component files here) |
| Thank-you page | `src/pages/v3/thank-you.astro` (create this) |
| Font pairing | DM Sans (sans) + Lora (serif) -- already configured in the layout |

The layout scaffold at `src/layouts/LayoutV3.astro` already has fonts, meta tags, and the HTML shell. Do not modify it unless you need to add something to the `<head>`. Build everything else as components imported into the page file.

---

## 2. REFERENCE IMAGES

**Folder:** `_staging/references/v3/`

Files: `v3a.png`, `v3b.png`, `v3c.png`, `v3d.png`, `v3f.png`, `v3g.png`
...
- Links to: `/v3/thank-you`
- Lower friction entry point for prospects not ready to book a call

**Thank-you page (`src/pages/v3/thank-you.astro`):**
- Create this page matching the v3 design aesthetic
...
- Include a "Back to home" link to `/v3`
...
- **Visual:** Use `_staging/graphics/proof-cards-stack-minimal-01.jpg` as the hero proof base.
...
- **Hero composition rule:** Treat `proof-cards-stack-minimal-01.jpg` as a source image to crop, mask, or recompose. Remove its built-in pale backdrop feel so the Google Maps / reviews / page-one proof cards appear to float directly on the page atmosphere.
...
Use these designated graphics for `v3` rather than choosing freely from the whole bucket:
- **Hero proof source:** `_staging/graphics/proof-cards-stack-minimal-01.jpg`
- **Social proof / stat strip source:** `_staging/graphics/stats-bar-calls-rating-ranking.png`
- **Review engine proof source:** `_staging/graphics/verified-review-growth-timeline-01.jpg`

Asset handling rules:
- Copy them to `public/images/` with version-prefixed names such as `v3-hero-proof.jpg`, `v3-stats-strip.png`, and `v3-review-proof.jpg`.
- Do not reuse the v4 or v5 designated hero images in this version.
- If the raw source includes a baked background panel or slab, crop or rebuild it so the final composition still feels transparent and integrated with the page.
...
Use `logo-wordmark-minimal-thin-domain.jpg` for `v3`, then copy it to `public/images/` as `v3-logo.jpg`.
...
- **Architecture:** Self-contained version. Own layout, own components in `src/components/v3/`. Do not import from `src/components/sections/` or `src/components/v2/` — those belong to other versions.
- **Layout:** The layout scaffold already exists at `src/layouts/LayoutV3.astro` with fonts loaded. Replace the page content, do not modify the layout unless you need to add something to the `<head>`.
...
1. Update `VERSIONS.md` in the project root:
   - Change v3 status from "Scaffold" to "Complete"
...
2. Update the hub page `src/pages/index.astro`:
   - Change the v3 card badge from "Scaffolded" to "Live"
